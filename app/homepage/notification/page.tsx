...
const NotificationPage = () => {
    ...
    const deleteAllNotifications = async () => {
        try {
            setLoading(true);
            message.loading('Đang xóa notifications...', 0);

            // Call API xóa tất cả notifications của user
            await deleteNotificationsByUserId(user.user._id!);
            ...
            message.destroy();
            message.error('Lỗi khi xóa notifications');
    ...
    // 🗑️ TEMP: Hàm refresh data (reload lại từ server)
    const refreshNotifications = async () => {
        try {
            setLoading(true);
            message.loading('Đang tải lại dữ liệu...', 0);
    ...
    // 🗑️ TEMP: Hàm xóa notifications cũ (giữ lại 5 cái mới nhất)
    const deleteOldNotifications = async () => {
        try {
            setLoading(true);
            message.loading('Đang xóa notifications cũ...', 0);

            // Call API xóa notifications cũ, giữ lại 5 cái mới nhất
            await API_NodeJS.delete(`notification/user/${user.user._id!}/old?keep=5`);
            ...
            message.destroy();
            message.error('Lỗi khi xóa notifications cũ');
            console.error('Delete old error:', error);
        } finally {
            setLoading(false);
        }
    };
